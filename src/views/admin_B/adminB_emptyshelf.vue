  ...
  emptySpaces: item.emptySpaces
      ...
      const checkshelfrequire =async()=>{
        try{
          ...
          const response =await axios.get(`http://121.36.23.117.223:3000/api/bookshelf/school/application_G`)
          if(response.status){
    ...
    return originData.filter(item => {
      return item.pressmark.includes(search.value) || 
         item.location.includes(search.value) 
    })
  })
